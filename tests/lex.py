...
- 贴图验证：python .\tests\lex.py textures
...
TEXTURE_SMOKE_SCRIPT = PROJECT_ROOT / "tests" / "texture_smoke.gd"
GODOT_CACHE_DIR = PROJECT_ROOT / ".godot"
GODOT_CLASS_CACHE = GODOT_CACHE_DIR / "global_script_class_cache.cfg"
...
def ensure_godot_imported() -> int:
    """干净克隆没有 .godot 缓存时，先让 Godot 扫描脚本和导入资源。"""

    if GODOT_CLASS_CACHE.exists():
        return 0
    godot = find_godot(console=True)
    return run_command([str(godot), "--headless", "--editor", "--path", str(GODOT_PROJECT_DIR), "--quit"])
...
def run_game() -> int:
    ...
    import_code = ensure_godot_imported()
    if import_code != 0:
        return import_code
    godot = find_godot(console=False)
...
def validate_textures() -> int:
    """用 headless Godot 直接加载关键战斗贴图，避免贴图缺失被普通启动验证漏掉。"""

    import_code = ensure_godot_imported()
    if import_code != 0:
        return import_code
    godot = find_godot(console=True)
    return run_command([str(godot), "--headless", "--path", str(GODOT_PROJECT_DIR), "--script", str(TEXTURE_SMOKE_SCRIPT)])
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "command",
        choices=("editor", "run", "validate", "textures", "build", "desktop"),
        help="editor=打开 Godot 编辑器，run=运行项目，validate=无头加载验证，textures=贴图加载验证，build=导出 Windows 数据包，desktop=运行已构建 exe",
    )
