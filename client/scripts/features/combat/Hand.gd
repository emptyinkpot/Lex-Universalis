...
@export var hand_card_rotation_curve: Curve = preload("res://assets/curves/hand_rotation_curve.tres")
...
@export var hand_card_y_offset_curve: Curve = preload("res://assets/curves/hand_y_curve.tres")
...
func _on_card_selected(card: Card):
	...
	if current_card_pick_action == null:
		_try_play_or_prompt_card(card)
	else:
		### picking
		attempt_pick_card(card)

func _on_card_drag_released(card: Card, release_position: Vector2) -> void:
	if current_card_pick_action != null:
		attempt_pick_card(card)
		tween_hand()
		return
	var play_line_y: float = global_position.y - max(160.0, size.y * 0.75)
	if release_position.y <= play_line_y:
		_try_play_or_prompt_card(card)
	tween_hand()

func _try_play_or_prompt_card(card: Card) -> void:
	### playing
	...
	if card.card_data.card_requires_target:
		...
		_queue_card_without_target(card)

func _queue_card_without_target(card: Card) -> void:
