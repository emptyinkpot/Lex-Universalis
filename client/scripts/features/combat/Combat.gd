...
@onready var deck_button: TextureButton = $BattleLayout/TopBar/DeckButton
@onready var draw_pile_button: Button = %DrawPileButton
@onready var discard_pile_button: Button = %DiscardPileButton
@onready var exhaust_pile_button: Button = %ExhaustPileButton
...
var end_turn_object: CombatEndTurnScript = null
...
func update_combat_display():
	...
	draw_pile_button.text = "抽牌堆\n" + str(len(Global.player_data.player_draw))
	discard_pile_button.text = "弃牌堆\n" + str(len(Global.player_data.player_discard))
	exhaust_pile_button.text = "消耗堆\n" + str(len(Global.player_data.player_exhaust))
...
func set_combat_display_visibility(display_visibility: bool) -> void:
	%PlayerStatsBar.visible = display_visibility
	%BottomBar.visible = display_visibility
...
func _on_end_turn_button_up():
	queue_end_turn(CombatEndTurnScript.END_TURN_QUEUE_IMMEDIACY.WAIT_FOR_ALL_CARD_PLAYS)

func _on_end_turn_requested(immediacy: int):
	queue_end_turn(immediacy)

func queue_end_turn(immediacy: int = CombatEndTurnScript.END_TURN_QUEUE_IMMEDIACY.WAIT_FOR_ALL_CARD_PLAYS):
	# queues up an end turn, using async objects with priority to determine how to handle it
	if end_turn_object == null:
		end_turn_object = CombatEndTurnScript.new(self, %Hand, immediacy)
