...
signal card_drag_released(Card, Vector2)

var _drag_pointer_down: bool = false
var _is_dragging: bool = false
var _drag_start_position: Vector2 = Vector2.ZERO
var _drag_visual_start_position: Vector2 = Vector2.ZERO
...
func _on_button_gui_input(event: InputEvent):
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			_start_drag_pointer(get_global_mouse_position())
		else:
			_finish_drag_pointer(get_global_mouse_position())
	if event is InputEventScreenTouch:
		if event.pressed:
			_start_drag_pointer(event.position)
		else:
			_finish_drag_pointer(event.position)
	if _drag_pointer_down and event is InputEventMouseMotion:
		_update_drag_pointer(get_global_mouse_position())
	if _drag_pointer_down and event is InputEventScreenDrag:
		_update_drag_pointer(event.position)
	if event.is_action_pressed("right_click"):
		card_right_clicked.emit(self)

func _start_drag_pointer(pointer_position: Vector2) -> void:
	_drag_pointer_down = true
	_is_dragging = false
	_drag_start_position = pointer_position
	_drag_visual_start_position = global_position

func _finish_drag_pointer(pointer_position: Vector2) -> void:
	if _is_dragging:
		card_drag_released.emit(self, pointer_position)
	else:
		card_selected.emit(self)
	_drag_pointer_down = false
	_is_dragging = false
	global_position = _drag_visual_start_position

func _update_drag_pointer(pointer_position: Vector2) -> void:
	if not _is_dragging and pointer_position.distance_to(_drag_start_position) >= DRAG_START_DISTANCE:
		_is_dragging = true
		z_index = 100
	if _is_dragging:
		global_position = _drag_visual_start_position + pointer_position - _drag_start_position
